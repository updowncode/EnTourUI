...
fs.copyFileSync('dist/EnTourUI/polyfills.js', 'dist/EnTourUI/Scripts/polyfills.js')
...
fs.readdir('dist/EnTourUI/assets', function(err, items) {
    for (var i=0; i<items.length; i++) {
        ...
        var dest = 'F:/ENDev.TOUREAST.COM/DesktopModules/EnTourModule/assets/' + items[i];
...
fs.readdir('dist/EnTourUI/scripts', function(err, items) {
    for (var i=0; i<items.length; i++) {
        ...
        var dest = 'F:/ENDev.TOUREAST.COM/DesktopModules/EnTourModule/scripts/' + items[i];
        fs.copyFileSync(source, dest)
    }
});
